...
interface Course {
  ...
  description: string;
  subjectIds: string[];
  subjects?: Subject[]; // Hydrated subjects
}
...
export default function Page() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const subjectList = subjectsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Subject));
      ...
      const courseList = await Promise.all(
        coursesSnap.docs.map(async docSnap => {
          ...
          const hydratedSubjects: Subject[] = [];
      ...
      <nav className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="hidden sm:flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm font-medium text-sm"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200 font-medium text-sm"
          >
...
function CourseListView({ courses, onSelect, onDelete }: { courses: Course[], onSelect: (c: Course) => void, onDelete: (id: string) => void }) {
  return (
    <motion.div
    ...
    >
      {courses.map(course => (
        <div
          key={course.id}
          className="group relative bg-white rounded-2xl p-6 border border-slate-200 hover:border-indigo-300 hover:shadow-xl hover:shadow-indigo-100/50 transition-all duration-300 cursor-pointer"
        ...
        >
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-slate-500 line-clamp-2">{course.description || 'No description provided.'}</p>
          </div>
...
function CourseDetailView({
  ...
  return (
    ...
    >
      {/* Tabs */}
      <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 w-fit backdrop-blur-sm sticky top-24 z-20 shadow-sm">
...
function GeneralTab({ course, refreshData }: { course: Course, refreshData: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl">
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
        <h3 className="text-xl font-bold text-slate-800 mb-6">General Information</h3>
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={5}
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all outline-none resize-none"
            />
          </div>
...
function SubjectsTab({ course, allSubjects, refreshData }: { course: Course, allSubjects: Subject[], refreshData: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col lg:flex-row gap-6">
      <div className="flex-1 bg-white rounded-2xl p-6 shadow-sm border border-slate-200 h-fit">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-slate-800">Connected Subjects</h3>
          <button
            onClick={handleSave}
      ...
      <div className="flex-1 bg-slate-50 rounded-2xl p-6 border border-slate-200/50">
        <h3 className="text-lg font-bold text-slate-800 mb-4">Preview</h3>
        <p className="text-sm text-slate-500 mb-6">This is how the subjects are structured within the course.</p>
        <div className="space-y-4">
          {allSubjects.filter(s => selectedIds.includes(s.id)).map(sub => (
            <div key={sub.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              <strong className="block text-indigo-900 font-semibold mb-2">{sub.name}</strong>
              <div className="flex flex-wrap gap-2">
                {Object.keys(sub.chapters || {}).map(ch => (
                  <span key={ch} className="px-2 py-1 bg-slate-100 text-slate-600 text-xs rounded-md font-medium">
                    {ch}
                  </span>
                ))}
          ...
          {selectedIds.length === 0 && (
            <p className="text-center text-slate-400 italic py-8">No subjects selected</p>
          )}
        </div>
      </div>
    </motion.div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-slate-800">Series Management</h3>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {seriesList.map(item => (
          <div key={item.id} className="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-lg transition-all group">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider bg-indigo-50 px-2 py-1 rounded-md">{item.year}</span>
                <h4 className="text-lg font-bold text-slate-900 mt-2">{item.name}</h4>
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  className="p-1.5 hover:bg-red-50 rounded-md text-slate-500 hover:text-red-600"
                >
                  <Trash className="w-4 h-4" />
                </button>
              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      ...
      >
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h2 className="text-xl font-bold text-slate-800">{seriesToEdit ? 'Edit Series' : 'Create New Series'}</h2>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input
                className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-mono text-sm"
        ...
        <div className="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
          <button onClick={onClose} className="px-5 py-2.5 text-slate-600 hover:bg-slate-200 rounded-lg font-medium transition-all">Cancel</button>
          <button
...
function CreateCourseModal({ isOpen, onClose, onSuccess, subjects }: { isOpen: boolean, onClose: () => void, onSuccess: () => void, subjects: Subject[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto custom-scrollbar border border-slate-100 rounded-lg p-2">
              {subjects.map(sub => (
                <label key={sub.id} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded-md cursor-pointer">
                  <input
                    type="checkbox"
...
function ManageSubjectsModal({ isOpen, onClose, subjects, onUpdate }: { isOpen: boolean, onClose: () => void, subjects: Subject[], onUpdate: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 flex flex-col max-h-[85vh]">
        ...
        <div className="pt-4 mt-4 border-t border-slate-100 text-right">
          <button onClick={onClose} className="text-slate-500 hover:text-slate-800 font-medium">Close</button>
        </div>
      </motion.div>
    </div>
  );
