    ...
    return (
        <div className="p-8 flex gap-6 h-full">
            ...
            <div className="flex-1 space-y-6">
                <div>
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">My Assigned Tasks</h1>
                    <p className="text-muted-foreground mt-1">Manage tasks assigned to you by Super Admin</p>
                </div>
                    ...
                    {tasks.map(task => (
                        <Card
                            ...
                            className={cn("cursor-pointer transition-all hover:shadow-md border-l-4",
                                ...
                                task.status === 'completed' ? "border-l-emerald-500 opacity-75" : "border-l-blue-500"
                            )}
                        ...
                        >
                            ...
                            <CardContent>
                                ...
                                <div className="flex items-center gap-4 mt-3 text-xs font-medium">
                                    <span className="flex items-center text-emerald-600"><DollarSign className="w-3 h-3 mr-1" />{task.paymentAmount}</span>
                    ...
                    <div className="mb-6 pb-6 border-b">
                        ...
                        <div className="mt-6 flex flex-col gap-2">
                            ...
                            {selectedTask.status === 'in_progress' && (
                                <Button onClick={() => handleStatusUpdate(selectedTask.id, 'completed')} className="w-full bg-emerald-600 hover:bg-emerald-700">Mark as Completed</Button>
                            )}
                            {selectedTask.status === 'reviewed' && (
                                <div className="bg-yellow-50 p-3 rounded-lg mt-2 text-sm">
                                    <strong>Super Admin Review: </strong> {selectedTask.reviewFeedback} ({selectedTask.rating}★)
                                </div>
                            )}
                        </div>
                    </div>
